...
Note that, the larger $\eta_x$ is \MZ{"is what?"} (where comparison is made dimension-wise), the smaller is the state space $\Xh$ resulting in an efficient computation.
...
In order to be able to break Problem~\ref{prob:reach-avoid} into synthesizing controller $C^{\nom}$ and solving for Problem~\ref{prob:tracking_with_time}, we need to augment the state vector of $\Sigma$ with additional state variable time.
Adding time as a state variable, the corresponding augmented control system is represented as $\widetilde{\Sigma}= (\widetilde{X},U,W,\widetilde{f})$ with $\widetilde X= X\times \reals_{\geq 0}$ \MZ{"Time is also discretized"} and $\widetilde f=\begin{bmatrix} f^T& 1\end{bmatrix}^T $.
...
We introduce some notation.
Given a state $\widetilde x=\begin{bmatrix} x &t \end{bmatrix}^T$ of a control system $\widetilde \Sigma$, and given a positive real number $\varepsilon$, we use the notation $\ball_\varepsilon(\widetilde x)$ to denote the ball (with infinity norm) of radius $\varepsilon$ centered around $x$ and radius zero for time $t$.
Formally, \[\ball_\varepsilon(\widetilde{x}=\begin{bmatrix} x^T &t \end{bmatrix}^T):= \set{\widetilde{x}'=\begin{bmatrix}x' & t' \end{bmatrix}^T
	\in \widetilde{X}\mid  (x'^T \in \ball(x)) \land (t=t')}.\] 
