...
\begin{algorithm}
	...
	\begin{algorithmic}[1]
		\Require $\widetilde{\Sigma}=(\widetilde{X},U,W,\widetilde{f})$, $\tau \in \mathbb{R}_{>0}$, $\eta_x\in \mathbb{R}^n_{>0}$, $\eta_u\in \mathbb{R}^m_{>0}$, $((x_{0_\nom},0),\ldots,(x_{K_\nom},K))$, $\varepsilon \in \mathbb{R}_{>0}^{n}$
