...
%h^v(x(t))=x(t).
%\]
%\RM{shouldn't disturbance model be given up front?Shouldn't $\varepsilon$ be an input to ALTRO?}
...
Tab.~\ref{tab:runtimes} illustrates list of runtimes and number of input-state pairs corresponding to local and global ABCD. 
When the number of agents $N>1$, the memory requirement for global ABCD exceeds our limits (1.5TB of RAM). 
Fig.~\ref{fig:formation_ex} illustrates four sequential frames of a sample trajectory generated by employing feedback controllers. Notice that both of the relative position and orientation between drones are kept (almost) constant 
