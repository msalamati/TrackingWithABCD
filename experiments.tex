...
\begin{equation*}\label{eq:unicycle_ss}
	...
	\end{bmatrix}.
\end{equation*}
where $x_1$ and $x_2$ denote the drone's position in two-dimensional space, $x_3$ denotes the rotational angle, and $u_1$ and $u_2$ represent control inputs for each drone. Choosing a sampling time $\tau=0.1s$, the nominal dynamics $f^d_\tau$ can be characterized uniquely. %Further, the output mapping is defined as
%\[
...
On fixing the sampling time $\tau=0.1s$, one can derive $f^c_\tau$ and $f^l_\tau$. %The output mapping for the crane is defined as
%\[
%h^l=\begin{bmatrix}
%	x_1^{(2)}\\
%	y_2
...
%h^v(x(t))=x(t).
%\]
\RM{shouldn't disturbance model be given up front?
Shouldn't $\varepsilon$ be an input to ALTRO?}
...
\RM{why is this surprising:} Surprisingly, both of the relative position and orientation between drones are kept (almost) constant 
