...
\begin{table*}
	...
	\resizebox{1\columnwidth}{!}{
		\begin{tabular}{l|c|ccc|ccc}
			...
			&$T_{AL}$&$|Domain|$&$T_{l}^{abs}$&$T_{l}^{syn}$&$T_g^{abs}$&$T_g^{syn}$\\
			\midrule
			
			\multirow{1}{*}{\rotatebox{0}{Multi-drone path planning}} & $125$ & $4\times 10^5$ & $91.0$  & $9.22$& $7.14\times 10^{70}$&OM&OM \\
	...
	\item The static obstacle $\obs\subseteq X$ is defined as 
		\begin{align}
			&\obs \coloneqq\nonumber\\ 
				&\Set{ x\in X^\times | 
					\begin{array}{c}
						\exists i\in [1;N]\;.\; D(\proj^i(x),\proj^i(\avoid)) \leq \delta\\
						\vee\\
