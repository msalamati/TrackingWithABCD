...
We consider two distinct types of problems, and demonstrate how they can be solved using our approach. Table~\ref{tab:runtimes} demonstrates the runtimes of different stages for each of the experiment. Note that while reported numbers for local controller synthesis correspond to the total runtimes over all of the robots, significant reduction would be expected if abstraction and synthesis phases were parallalized.

\begin{table*}
	\centering
	\caption{List of runtimes for different case-studies
		\textmd{Runtimes (in seconds) for computing the nominal controllers over the corresponding product spaces using ALTRO ($T_{AL}$), (total) abstraction and synthesis times for synthesizing local controllers for all of the robots using SCOTS ($T_{SC}^{abs}$ and $T_{SC}^{syn}$, respectively).}
	}
	\label{tab:runtimes}
	...
	\resizebox{1\columnwidth}{!}{
		\begin{tabular}{l|c|rr}
			...
			\multirow{1}{*}{\rotatebox{0}{multi-drone path planning}} & $125$ & $578$  & $68$ \\
			
			\midrule
			
			\multirow{1}{*}{\rotatebox{0}{crane and forklift}} & $0.65$ & $511.22$  & $91.03$\\
			
			\midrule
			
			\multirow{1}{*}{\rotatebox{0}{lane merging}} & $100$ & $195$  & $38.5$\\
			\midrule
			\multirow{1}{*}{\rotatebox{0}{multi-drone formation control}} & $163$ & $570$  & $140$\\
			\bottomrule
	\end{tabular}}
\end{table*}
...
In the next step, we use SCOTS in order to compute a feedback controller tolerating additive disturbance $|W^1|\leq\begin{bmatrix}0&0.03&0&0\end{bmatrix}^T$ for the crane and $|W^2|\leq\begin{bmatrix}0&0.1\end{bmatrix}^T$ for the forklift. %We note that the selection of tube sizes need to satisfy the following inequality:
...
We choose state and input spaces for the crane to be $X^{1}=[0,5.06]\times[-0.59,2.18]\times[1.66,4.14]\times[-4.7,4.45]$ and $U^{1}=[-7,7]$, respectively. For the forklift, we set $X^{2}=[-3,3]\times[2,9]$ and $U^{2}=[-3,3]$. %Next, we augment both dynamics with time and 
