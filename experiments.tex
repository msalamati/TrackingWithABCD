...
We consider two distinct types of problems, and demonstrate how they can be solved using our approach. Table~\ref{tab:ipd} demonstrates the runtimes of different stages for each of the experiment.
...
In this example, we consider a planning scenario for ten identical drones ($N=10$). A set of initial and target states for each drone are specified and depicted in Fig.~\ref{fig:MA}. The drones must avoid hitting a physical obstacle that is depicted with green circle in Fig.~\ref{fig:MA}. %A physical obstacle is added to the problem over the $2D$ plane, depicted in Figure~\ref{fig:MA} with a yellow circle. 
The threshold for minimum distance from the static obstacle and other drones is chosen to be $\delta=\begin{bmatrix}0.3&0.3\end{bmatrix}^T$. %and $\delta_{obs}=\begin{bmatrix}2&2\end{bmatrix}^T$.
...
By allowing disturbance to take non-zero values, there is no guarantee that the open-loop trajectory is followed and minimum distances are kept above the design thresholds. Fig.~\ref{fig:multi_drone_distance} illustrates performance of open-loop and feedback controllers on regulating distance between the first and second drones ($d^{p}(h^1(\proj^1(x_{nom})),h^2(\proj^2(x_{nom})))$) with and without disturbance. As expected, in the absence of disturbance open-loop controller suffices and distance between the two drones (shown in solid blue) does not go below the threshold line $\delta=0.28$. Next, we consider the case when constant additive disturbance $w=\begin{bmatrix}0.03&0.03&0.03\end{bmatrix}^T$ is being applied to both of the drones throughout the whole horizon. It can be noticed that applying the open-loop controller causes that distance between the two drones (shown in solid red) becomes smaller than $\delta$. However, the feedback controller is still capable of maintaining distance above the given threshold (shown in solid green).
\begin{figure}[t]
	\centering
...
\begin{figure}[t]
	\centering
	\includegraphics[width=0.45\textwidth]{figures/distances.pdf}
...
We choose state and input spaces for the crane to be $X^{1}=[0,5.06]\times[-0.59,2.18]\times[1.66,4.14]\times[-4.7,4.45]$ and $U^{1}=[-7,7]$, respectively. For the forklift, we set \MS{$X^{2}=??$} and $U^{2}=[-3,3]$. %Next, we augment both dynamics with time and 
...
\begin{itemize}
	...
	\item The static obstacle $\obs\subseteq X$ is defined as 
		\begin{align}
			...
			&\Set{ x\in X | 
				\begin{array}{c}
					\exists i\in [1;N]\;.\; d^{p}_e(h^i(\proj^i(x)),\obs) \leq \delta\\
					\wedge\\
					\exists (i,j)\in [1;N]\times [1;N]\;.\;\\ d^{p}\left(h^i(\proj^i(x)),h^j(\proj^j(x))\right)\notin\lambda^{i,j}\pm \mu
			\end{array}},					
		\end{align}
\end{itemize}
...
\begin{figure}[t]
	...
	\caption{Illustration of the reference trajectories generated by ALTRO for the robot formation control example}
...
\begin{table*}
	...
	\textmd{Runtimes (in seconds) for computing nominal controller over the product space using ALTRO ($T_{AL}$), (total) abstraction and synthesis times for computing local controllers for all of the robots using SCOTS ($T_{SC}^{abs}$ and $T_{SC}^{syn}$, respectively).}
	}
	\label{tab:ipd}
	...
	\resizebox{1\columnwidth}{!}{
		\begin{tabular}{l|c|rr}
			\toprule
			&\multicolumn{1}{c}{central synthesis}&
			\multicolumn{2}{|c}{local synthesis}\\
			
			case-studies&$T_{AL}$&$T_{SC}^{abs}$&$T_{SC}^{syn}$\\
			\midrule
			
			\multirow{1}{*}{\rotatebox{0}{multi-car}} & $125$ & $578$  & $68$ \\
			
			\midrule
			
			\multirow{1}{*}{\rotatebox{0}{crane and forklift}} & $0.65$ & $512.1$  & $91.16$\\
			
			\midrule
			
			\multirow{1}{*}{\rotatebox{0}{lane merging}} & $100$ & $195$  & $38.5$\\
			\midrule
			\multirow{1}{*}{\rotatebox{0}{formation control}} & $163$ & $570$  & $140$\\
			\bottomrule
	\end{tabular}}
\end{table*}
