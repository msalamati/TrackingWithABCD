...
\begin{description}
	...
	We assume that the set of local state variables of all the systems are pairwise disjoint. \MZ{this part is ambiguous to me. I don't understand Z}
...
\begin{itemize}
	...
	\begin{itemize}
		...
		\item for every $(i,j)\in [1;N]\times [1;N]$ and for every $k\in [0;K]$, $e(x_{k_\nom}^i[X^i\cap Z], x_{k_\nom}^j[X^j\cap Z]) > \delta_\col$. \MZ{I know it is about collision avoidance but I can't understand the notation(for example why intersection with $Z$?)}
	\end{itemize}
...
The first inequality is there to ensure that $\varepsilon$ is large enough that a \emph{continuous} tube can be constructed by stitching together $\varepsilon$-neighborhoods around the sampled nominal trajectory points, as otherwise there could be ``gaps'' between some consecutive $\varepsilon$-neighborhoods. \MZ{ what is the problem with gaps ? }
The second inequality is there to ensure that there exists a goal state, so ``deep'' within the set $\goal$, that its whole $\varepsilon$-neighborhood is included in $\goal$, so that a tracking controller surely satisfies the original specification. \MZ{with current implementation that domain includes all of the points in target set it shouldn't be a problem  }
