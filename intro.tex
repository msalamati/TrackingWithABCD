...
We assume that the robots have been assigned a joint reach-avoid task, where the robots need to collectively reach a set of target states while collectively avoiding a set of static obstacles.
...
To circumvent these issues, in this paper, we propose a semi-centralized hierarchical controller synthesis approach involving ABCD.
In the top layer, we use some off-the-shelf fast controller synthesis method to make a joint global plan using the aforementioned centralized approach.
In this paper, we use ALTRO \cite{howell2019altro} in the top layer, which is an optimal-control-based controller synthesis tool that scales quite well to large dimensional system.
Unfortunately, the fast and scalable tools, including ALTRO, usually do not provide formal guarantees against external disturbances or modeling uncertainties.
So in the bottom layer, we use ABCD to synthesize formally verified \emph{tracking} controller for each robot for following the projection of the joint plan on the robot's state space.
...
The scenario is illustrated in Fig.~\ref{fig:cr_and_lft}; the four different subfigures from top to bottom will be referred to as Frame~1-4 respectively.
...
Next, we use the tool ALTRO to obtain a \emph{global} open-loop controller for the product system.
(An open-loop controller is one that bases its control decision on the current time instant.)
ALTRO is a scalable and fast controller synthesis tool that is suitable for the possibly high dimension of the product system.
...
Actually, Fig.~\ref{fig:cr_and_lft} shows various moments in the simulation of the local open-loop controllers in the presence of the disturbances, and Frame~3 shows that the crane and vehicle violate the constraint of the minimum safe distance.

To deal with this robustness issue, at the \textbf{bottom level} of our hierarchical approach, we employ ABCD to ``robustify'' the local open-loop controllers against disturbances.
More concretely, we consider the open-loop controlled trajectory of \emph{each system in isolation}, and use ABCD to synthesize a \emph{tracking controller}.
...
\begin{figure}[t]
	...
	\caption{Illustration of the reference trajectories produced by ALTRO for the crane and the factory vehicle example}
