...
%
In multi-robot scenarios, the inequality constraints can be used to define collision and obstacle avoidance specifications and the equality constraints can define fixed formation specification. \MS{Note that the reach-avoid specification is fulfilled if the corresponding equality and inequality constraints (i.e., $g(\cdot)\leq 0$, $h(\cdot)=0$) are satisfied at every time-step and thus choice of the quadratic objective function ($\ell_k$ for $k\in[0;T]$) is not crucial.}
