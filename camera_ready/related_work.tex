...
Unlike us, they neither consider external disturbances nor provide formal guarantees. \MS{Pant et al. considered STL expressable multi-quadrotor missions \cite{Pant2018multiquad}. They find the reference trajectory by maximizing robustness with respect to the given STL specification, and synthesize tracking controllers so that the reference trajectory is followed closely. Although their method provides some robustness margin, it is not capable of providing guarantee against worst-case bounded additive disturbances as we do. Further, they only consider time-bounded STL specifications.}	
	%In the first step, they generate a reference trajectory by maximizing robustness with respect to the given STL specification. In the next step, local MPC controllers are used to track the reference trajectory. Their implementations show that their method is tractable and as a plus gives gurantees over the continuous-time trajectory. However, their method is only able to handle bounded-time specifications and more importantly, they do not consider modelling uncertainty unlike us.}
...
Moreover, they only consider simple linear systems (double integrator) as vehicle models. \MS{Nikou et al. studied the problem of robust navigation for multi-agent systems \cite{Nikou2019}. 
