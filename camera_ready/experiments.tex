...
We have implemented our approach (as presented in Sec.~\ref{sec:problem}) in the tool called \tool, which is freely available online: \KM{Add the link}.
...
Selecting the horizon length as $T=102$ and minimum safe distance as $\delta=0.28$, ALTRO computes a valid open-loop trajectory %$x_{nom}$
...
Already for two drones, the memory requirement for global ABCD exceeds the available memory resource (1.5TB of RAM).
\KM{Update the RAM if you run the experiment again on desktop.}
