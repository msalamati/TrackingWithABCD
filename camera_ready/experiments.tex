...
We evaluate an implementation of Alg.~\ref{alg:main}
...
\begin{figure}[t]
	...
	\caption{Variations of run times of local ABCD among different agents for three case studies. \MS{add time unit to the vertical axis.}}
	\label{fig:box_plot}
\end{figure}
...
We consider a planning scenario for ten identical drones ($N=10$). A set of initial and target states for each drone is depicted in Fig.~\ref{fig:MA}. The drones must avoid hitting a physical obstacle that is the green square in the center of Fig.~\ref{fig:MA}. %A physical obstacle is added to the problem over the $2D$ plane, depicted in Figure~\ref{fig:MA} with a yellow circle. 
...
%Further, the output mapping is defined as
%\[
...
Selecting the horizon length as $T=102$ and minimum safe distance as $\delta=0.28$ (chosen to be compatible with $\varepsilon$), \KM{@Mehrdad: If $\delta$ and $\varepsilon$ are independent of each other, then what does compatibility between them mean here?} ALTRO computes a valid open-loop trajectory %$x_{nom}$
...
%Note that $\delta_{col} > \varepsilon$.
 ...
 Fig.~\ref{fig:3dtubes} gives time-space illustration for the safe tubes around the nominal trajectories. %We want to robustify the nominal controller against disturbances with $|W|\leq \begin{bmatrix}0&0.025&0.025\end{bmatrix}^T$. 
...
Fig.~\ref{fig:multi_drone_distance} illustrates performance of open-loop and feedback controllers in regulating distance between two particular 
...
Next, we consider the case when constant additive disturbances $\begin{bmatrix}0 &0.025&0.025\end{bmatrix}^T$ 
and $\begin{bmatrix}0 &-0.025&-0.025\end{bmatrix}^T$ are being applied to the two drones throughout the whole horizon. 
%\RM{shouldn't the disturbance bound be stated earlier, before using SCOTS?}
...
%\RM{shouldn't the disturbance model be described before?}
%
There is no obstacle for this example and for minimum distance between the crane and the vehicle we choose $\delta=0.035$ (chosen to be compatible with $\varepsilon^1$ and $\varepsilon^2$). %$\delta=\begin{bmatrix}0.035&0.2\end{bmatrix}^T$. 
\KM{@Mehrdad: Again the same thing: what do you mean by having the $\varepsilon$-s compatible with $\delta$?}
Fixing the horizon length to $T=70$, ALTRO was capable of generating a valid nominal trajectory in $0.65$ seconds. Fig.~\ref{fig:cr_and_lft} (\textbf{left}) demonstrates snapshots of the produced trajectory. 
As before, under the nominal open-loop controllers, applying (constant) additive disturbance $W=\begin{bmatrix}0&0.05&0&0\end{bmatrix}^T$ (to the cart-pole system) causes collision between the crane and the vehicle before the end of the mission (Fig.~\ref{fig:cr_and_lft} (\textbf{right})).
%\RM{shouldn't $\varepsilon$ be described here?}

In the next step, we use SCOTS in order to compute a feedback controller tolerating disturbances $W^1$ and $W^2$. %We note that the selection of tube sizes need to satisfy the following inequality:
...
Tab.~\ref{tab:runtimes} illustrates list of run times and number of input-state pairs corresponding to local and global ABCD. As before, for the cart-pole model, memory requirement for global ABCD exceeds our limits (1.5TB of RAM). \MS{Note that computing feedback controllers for the crane and vehicle takes $??$ and $??$ seconds, respectively. The large difference is due to the difference in the size of transition systems for the two dynamics.} Note that, using ALTRO alone would not provide guarantee against bounded disturbance (Fig~\ref{fig:cr_and_lft}, \textbf{right}). 
...
%This reduces size of state space into $1.9\times 10^5$ (in average). Given the above settings, SCOTS was able to compute abstraction in $159.6$ seconds ($26.6$ seconds in average) and synthesize controllers in $40.8$ seconds ($6.8$ seconds in average).
...
Consider a formation control scenario where a set of five drones (identically modeled) need to go from a specified start point to a certain destination (both defined over the corresponding state spaces) within a finite horizon, while four of them forming a diamond around a fifth drone (positioned at the diamond's center) at every time point. There are two square obstacles from which the group needs to keep a certain minimum distance at all of the time points. %As mentioned before, to check the formation the relative positions between the $i^{th}$ and $j^{th}$ drones is measured by euclidean norm ($d_e^p(.,.)$) whereas collision/obstacle avoidance is checked by the infinity norm$d^p(.,.)$.

%The model for each of the drones is the same as the drone model given in Sec.~\ref{sec:Multirobot} ($\Sigma^i=(X,x_\init^i,U,W,f^d)$). 
...
Tab.~\ref{tab:runtimes} illustrates list of run times and number of input-state pairs corresponding to local and global ABCD. 
When the number of agents $N>1$, the memory requirement for global ABCD exceeds our limits (1.5TB of RAM). 
\KM{$N$ is constant (equal to 5), no?}
Fig.~\ref{fig:formation_ex} illustrates four sequential frames of a sample perturbed trajectory generated by employing feedback controllers. Notice that both 
...
%\RM{disturbance should go before the description of the algorithm's outputs}
As expected, in the absence of disturbances, the open-loop controllers suffice and the distance between the two drones (shown in solid blue) does not go below the threshold line (showed as the dotted line). 
