...
\KM{I changed the wordings a bit of the following example. But I don't like this example here. To me it fails to convey the idea of why a decomposition of the global reach-avoid task is not always possible. It is not clear what the states of the crane and vehicle physically represent and how their ``distance'' is measured using $D$ (i.e. how we can measure the distance between them by taking into account the height of the cart from the floor and the length of the pendulum rod). On the other hand, explaining all these missing things in detail would kill the purpose of a \emph{simple} motivating example. Consider the example that I have added after this one in highlighted text.}
\begin{example}
...
\new{
\begin{example}
	Consider two simple one-dimensional control systems $\Sigma^1=(X^1,x_\init^1,U^1,W^1,f^1)$ and $\Sigma^2=(X^2,x_\init^2,U^2,W^2,f^2)$ whose states evolve over the real line, and at every time step, the values of their states can be either incremented or decremented by $1$ independently.
	...
	Suppose the initial states of $\Sigma^1$ and $\Sigma^2$ are $x_\init^1=0$ and $x_\init^2=5$ respectively, and assume that the specification requires them to \emph{simultaneously} reach the respective goal sets $\goal^1=\set{10}$ and $\goal^2=\set{20}$ while \emph{maintaining a distance} of at least $5$ among themselves.
