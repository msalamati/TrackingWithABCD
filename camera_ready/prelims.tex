...
$\set{C^i}\triangleright \set{\Sigma^i}$ (resp.\ $\set{C^i}\parallel \set{\Sigma^i}$) \emph{realizes} the global control specification $\mathcal{L}$,
i.e., all trajectories of the respective systems belong to $\mathcal{L}$.
...
%\new{
	...
	This specification can be formulated as a reach-avoid objective $\left(\lnot\avoid\,\mathcal{U}\, \goal\right)$ for the overall two-dimensional product system, where $\avoid = \set{ (x^1,x^2)\in \mathbb{R}^2  \mid |x^1-x^2| < 5}$ and $\goal = \set{(10,20)}$.
