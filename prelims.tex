...
A \emph{transition system} $S=(Q,\Sigma,\delta)$ consists of a state space $Q$, an input space $\Sigma$, and a transition function $\delta:Q\times \Sigma \rightarrow 2^Q$. 
A system $S$ is \emph{finite} if $Q$ and $\Sigma$ are finite. 
A trajectory $\rho\in Q^\infty$ is a maximal sequence of states compatible with $\delta$:
for all $1\leq k < |\rho|$ there exists $\sigma\in \Sigma$ s.t.\ $\rho(k)\in \delta(\rho(k-1),\sigma)$ and 
if $|\rho| < \infty$ then $\delta(\rho(|\rho|),\delta)= \emptyset$ for all $\sigma\in \Sigma$.
