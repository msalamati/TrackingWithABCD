...
a compact \emph{disturbance set} $W\subset \mathbb{R}^n$ containing $0$, and
a \emph{nominal dynamics} $f:X\times U\rightarrow X$. 
...
Intuitively, a control system represents a (possibly nonlinear) dynamical system with control inputs $U$
and disturbance from the set $W$.
...
$\Sigma^\times = (X^\times, x_\init^\times, U^\times, \set{0^n}, f^\times)$,
...
\RM{actually I don't know how to do this.}
Consider $(X, x_\init, U, W, f)$ as above and
suppose that $f: X \times U \rightarrow X$ is such that $f(\cdot, u)$ is locally Lipschitz for all $u\in U$.
Given a time horizon $\tau$, an initial state $x_0$, and a constant input $u$, define the \emph{continuous time trajectory} of the system
on $[0, \tau]$ as an absolutely continuous function $\zeta: [0, \tau] \rightarrow X$ such that $\zeta(0) = x_0$, and
$\zeta$ satisfies the differential inclusion $\dot{\zeta} \in f(\zeta(t), u) + W$ for almost all $t\in [0,\tau]$.

Now, given a sampling time $\tau > 0$, define the \emph{sampled-time} control system $(X, x_\init, U, W, f_\tau)$ as the control
system for which $f_\tau(x, u)$ ...
\RM{here} 
...
The open-loop is obtained when we connect $C$ with $\Sigma$ serially, denoted by $C \triangleright \Sigma$. 
...
A \emph{control objective} $\mathcal{L}$ is a set of finite sequences in $(X^\times)^*$.
Intuitively, a control objective specifies a set of ``good behaviors'' of a system.
We consider \emph{reach-avoid} specifications, written as  $\lnot\avoid\; \mathcal{U}\; \goal$ in linear temporal logic, for two subsets 
...
The \emph{decentralized open-loop (resp. feedback) controller synthesis problem} asks, given $\set{\Sigma^i}$ and a control objective $\mathcal{L}$,
to construct a set of local open-loop (resp.\ feedback) controllers $C^i$ such that
the trajectories of $\set{C^i}\triangleright \set{\Sigma^i}$ (resp.\ $\set{C^i}\parallel \set{\Sigma^i}$ all belong to the control objective $\mathcal{L}$.
