...
%\textcolor{blue}{
%We use $d(x,y)$ to denote geometric distance between two points $x$ and $y$, and
%use $D(x,Y)$ to denote the geometric distance between the point $x$ and a set $Y$, defined as: $D(x,A):=\inf_y\set{d(x,y)\mid y\in Y}$.
%\KM{Before, the set was a polytope: does it have to be a polytope at this point? Also, by geometric distance between $x$ and $y$, do you mean the infinity norm or the euclidean norm?}

Let $(S,d)$ be a metric space, where $S$ is a set and $d\colon S\times S\rightarrow \mathbb{R}$ is the metric on $S$.
For any set $R\subseteq S$ and any element $s\in S$, we generalize the metric $d$ to measure the ``distance'' between the set $R$ and the element $s$ as follows:
$d(s,R)\coloneqq \inf_{r\in R}\set{d(s,r)}$.
