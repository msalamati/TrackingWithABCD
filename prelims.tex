...
consists of a state space $X= \mathbb{R}^n$, a \emph{finite} input space $U\subseteq\mathbb{R}^m$, 
...
A \emph{transition system} $S=(X,U,F)$ consists of a state space $X$, an input space $U$, and a transition function $F:X\times U \rightarrow 2^X$. 
...
A state feedback controller, or simply a \emph{controller}, of $\Sigma_\tau$ is a function $C:X\rightarrow U$.
The controlled \emph{closed loop} is obtained when we connect $C$ with $\Sigma_\tau$ in feedback, and is formalized using the transition system $\Sigma_\tau\parallel C = (X,U,f_\tau^C)$ s.t.\ $x'$ is in $f_\tau^C(x,u)$ iff $x'$ is in $f_\tau(x,u)$ and $C(x)=u$.
The controller $C$ can be interpreted as a zero-order-hold controller for the control system $\Sigma$.

Suppose $x_0$ be a given initial state of $\Sigma_\tau$.
The \emph{closed-loop behavior} $\Beh(x_0)$ of $\Sigma_\tau\parallel C$ is the set of all infinite state sequences $(x_0,x_1,\ldots)$ s.t.\ for all $i\geq 0$, there exists an input $u$ with $x_{i+1}\in f_\tau^C(x_i,u)$.
